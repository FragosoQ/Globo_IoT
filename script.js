...
// **IMPORTAÇÃO DOS DADOS DO NOVO FICHEIRO**
import { arcsData, pontosAdicionais } from './globe-países.js';

// Combina TODOS os arcos (ambas as listas)
const todosOsArcos = arcsData.concat(pontosAdicionais);
...
  lat: d.endLat,
  lng: d.endLng,
  size: d.name === 'Lisbon' || d.name === 'Porto' ? 0.25 : 0.2, // Exemplo: destacar Lisboa e Porto
...
  .arcsData(todosOsArcos) // <--- Corrigido para usar a lista combinada
  .arcColor(d => t => {
...
.arcStroke(1)   
  .pointsData(pointsData) // <--- Usando a lista de pontos correta (extraída dos arcos)
  .pointAltitude(0)
  .pointColor('color')
  .pointRadius(0.3)
  .labelsData(labelsData) // <--- Usando a lista de labels correta (extraída dos arcos)
  .labelColor('color')
  .labelAltitude(0.01)
  .labelSize('size')
